...
cpdef np.ndarray[np.float32_t, ndim=2] _ple_transform_cython(np.ndarray[np.float32_t, ndim=1] column_data, np.ndarray[np.float32_t, ndim=1] column_bin_boundaries, int num_bins):
    ...
    cdef np.ndarray[np.float32_t, ndim=2] encoded_data = np.ones((column_data.shape[0], num_bins), dtype=np.float32)
    ...
    cdef np.ndarray[np.float32_t, ndim=1] bin_widths = np.diff(column_bin_boundaries)
    ...
    cdef np.ndarray[np.float32_t, ndim=1] encoded_values = bin_numerator / bin_denominator
